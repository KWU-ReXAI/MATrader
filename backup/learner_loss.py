...
class TD3_Agent:
	...
	def update_models(self, iters, episode,noise, imitation_weight):
		std_value = iters % 4 + 2
		...
		if episode % std_value == 0:
			# network.actor_train으로 가중치를 넘겨줍니다.
			actor_loss = self.network.actor_train(states, imitation_actions, price, imitation_weight)
	...
	def run(self,max_episode=100, reward_n_step = 1,noise=0.001,start_epsilon=0.3):
		...
		for e in tqdm_e:
			...
			imitation_weight = max(0.0, 1.0 - (e / (max_episode - 1)))
			...
			while True:				
				...
				if len(self.n_steps_buffer) >= parameters.N_STEP_RETURNS:
					...
					self.update_models(e, episode, update_noise, imitation_weight)
				episode += 1
