...
class Trader:
    ...
    def act(self, action, stock_codes, f, recode):
        ...
        self.portfolio_value = self.balance + \
                               np.sum(curr_prices * self.num_stocks * (
                                           1 - parameters.TRADING_TAX - parameters.TRADING_CHARGE))
        ...
        if recode:
            for idx, curr_price in enumerate(curr_prices):
                f.write(str(date) + "," + str(stock_codes[idx]).zfill(6) + "," + str(curr_price) + "," + str(
                    action[idx]) + "," \
        ...
        return reward, mean_sharpe_reward