...
class KISApiHandler:
    ...
    def api_request(self, method, path, tr_id, params=None, data=None):
        ...
        try:
            ...
            logging.error(f"API 인증 실패: {e}");
            return None
        url = f"{self.url_base}{path}"
        headers = self.headers.copy()
        headers.update(
            {'authorization': f'Bearer {self.access_token}', 'appKey': self.app_key, 'appSecret': self.app_secret,
        ...
        if method.lower() == 'post' and data:
            try:
                ...
                logging.error(f"Hashkey 또는 POST 요청 실패: {e}");
                return None
        else:
            res = requests.get(url, headers=headers, params=params)
        if res.status_code != 200:
            logging.error(f"API 요청 실패: {res.status_code}, {res.text}");
            return None
        res_json = res.json()
