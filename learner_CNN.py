...
class TD3_Agent:
	def __init__(self, stock_codes:list, num_of_stock, phase, train_chart_data,test_chart_data, training_data, test_data,
		...
		self.inp_dim = self.training_data.shape[3] #학습 데이터 크기
		self.lr = lr
		self.balance = balance
		self.delayed_reward_threshold = delayed_reward_threshold
		self.batch_size = parameters.BATCH_SIZE
		# Create networks
		self.network = TD3_network(self.inp_dim, self.act_dim, lr, parameters.TAU, self.window_size, num_of_stock=self.act_dim)
